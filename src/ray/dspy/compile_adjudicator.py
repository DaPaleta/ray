"""Build-time compile of the verdict-adjudicator prompt with DSPy.

Run with:

    python -m ray.dspy.compile_adjudicator [--dry-run] [--n-agreement N]
...
2. Evaluates the hand-written baseline prompt (`prompts.VERDICT_ADJUDICATOR_PROMPT`)
   on both sets with the two-sided metric.
...
5. Writes `prompts/adjudicator.compiled.json` (the artifact `subagents.py` loads)
   and `prompts/adjudicator.report.md` (a before/after table).
...
COMPILED_PATH = REPO_ROOT / "prompts" / "adjudicator.compiled.json"
REPORT_PATH = REPO_ROOT / "prompts" / "adjudicator.report.md"
...
def _fallback_payload(reason: str, baseline_result: dict | None, n_agreement: int, n_adversarial: int) -> dict:
    ...
    return {
        "prompt": prompts.VERDICT_ADJUDICATOR_PROMPT,
        "score": score,
...
def main(argv: list[str] | None = None) -> int:
    ...
    class AdjudicateVerdict(dspy.Signature):
    ...
    AdjudicateVerdict.__doc__ = prompts.VERDICT_ADJUDICATOR_PROMPT

    class Adjudicator(dspy.Module):
        def __init__(self):
            super().__init__()
            self.predict = dspy.ChainOfThought(AdjudicateVerdict)
    ...
    try:
        ...
        optimized_program = optimizer.compile(Adjudicator(), trainset=agreement_ds)
        ...
        # `Adjudicator.predict` is a ChainOfThought, which wraps its own inner
        # `predict` (a plain dspy.Predict) that holds the final signature.
        inner_predict = optimized_program.predict.predict
        base_instructions = inner_predict.signature.instructions or prompts.VERDICT_ADJUDICATOR_PROMPT
